...
int main(int argc, char **argv){
    ...
    for (auto it=sessionList.begin() ; it!=sessionList.end() ; ++it){
        ip::tcp::resolver::query queryA((*it)->host_name, (*it)->port_num);
        ip::tcp::resolver::iterator endpoint_iterator = (*it)->resolv.resolve(queryA);
        for (ip::tcp::resolver::iterator iter; endpoint_iterator != iter; endpoint_iterator++)
        {
            ...
            strcpy(s, str.c_str());
            int a,b,c,d;
            (*it)->port = endpoint_iterator->endpoint().port();
        ...
        ip::tcp::resolver::query query( socks_server, socks_port);
...
void connect_handler(const boost::system::error_code &ec, vector<shared_ptr<Console> >::iterator itc)
{
    ...
    auto h2 = boost::bind(&no_handler);
    if (!ec){    
        int port =  (*itc)->port;

        unsigned char reply[8];
        reply[0] = 4;reply[1] = 1;
        reply[2] = port/256;reply[3] = port%256;
        reply[4] = (*itc)->a;reply[5] = (*itc)->b;
        reply[6] = (*itc)->c;reply[7] = (*itc)->d;
        write( (*itc)->tcp_socket, buffer(reply,8));
        (*itc)->tcp_socket.async_read_some(buffer((*itc)->bytes, 8), h2 );
